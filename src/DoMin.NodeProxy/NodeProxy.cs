...
using DoMin.Node.Core;
using Microsoft.AspNetCore.Builder;
using Microsoft.AspNetCore.Http;
using Microsoft.Extensions.Logging;
using HttpMethods = DoMin.Node.Core.HttpMethods;

namespace DoMin.Node;
...
public class NodeProxy
{
    ...
    public Func<HttpContext, bool> ShouldCache { get; set; } = default!;
    ...
    public bool IsPortAvailable() => IsPortAvailable(Client.BaseAddress!.Port);
            ...
            if (shouldCache)
            {
                ...
                if (bytes.Length > 0)
                {
                    ...
                    var mimeType = response.Content.Headers.ContentType?.ToString()
                        ?? MimeTypes.GetMimeType(cacheKey);
